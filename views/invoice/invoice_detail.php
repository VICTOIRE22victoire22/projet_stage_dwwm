<!-- 
    Code HTML permettant d'afficher la page de détail d'un agent.
...
-->
    <h1 class='title_detail'>Détail de la facture</h1>

    <!-- Lien vers le formulaire d’ajout -->
    <?php if(in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
        <a href="/index.php?page=invoice/add" class="add-form">
    ...
    <section class="section-common">
        <table class="table_detail">
            <thead>
                <tr>
                    <th class="label">Numéro de facture:</th>
                    <th class="label">Date:</th>
                    <th class="label">Montant:</th>
                    <th class="label">Statut:</th>
                    <th class="label">N° de compte:</th>
                    <th class="label">N° de sous-compte:</th>
            ...
            <tbody>
                <tr>
                    <td class="value"><?=htmlspecialchars($invoice['invoice_number'])?></td>
                    <td class="value"><?=htmlspecialchars($invoice['invoice_date'])?></td>
                    <td class="value"><?=htmlspecialchars($invoice['invoice_amount'])?></td>
                    <td class="value"><?=htmlspecialchars($invoice['invoice_status'])?></td>
                    <td class="value"><?=htmlspecialchars($invoice['invoice_account_number'])?></td>
                    <td class="value"><?= empty($invoice['invoice_sub_account_number']) ? 'Aucun sous-compte relié à cette facture' : $invoice['invoice_sub_account_number']?></td>
                    <td class="value">
                        <a href="/index.php?page=provider/detail&id=<?= urlencode($invoice['invoice_provider_id']) ?>" class="table-link">
                            <?= htmlspecialchars($invoice['provider_name']) ?>
                        </a>
                    </td>
                    <?php if(in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
                        <td class="actions">
                            <span class="action-icon">
                                <a href="/index.php?page=invoice/edit&id=<?=$invoice['invoice_id'] ?>">
    ...
    <p class="return_link">
        <a href="/index.php?page=invoice/index&search=<?= urlencode($_GET['search'] ?? '') ?>&sort=<?= urlencode($_GET['sort'] ?? '') ?>&order=<?= urlencode($_GET['order'] ?? 'asc') ?>&page_number=<?= urlencode($_GET['page_number'] ?? 1) ?>&limit=<?= urlencode($_GET['limit'] ?? 10) ?>" class="return_link-detail">
