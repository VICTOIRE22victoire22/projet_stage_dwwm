    ...
    <section class="form-container">
        <form method="POST" action="">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-input" id="invoice_number" name="invoice_number" value="<?= htmlspecialchars($number) ?>" required>
            </div>

            <div class="form-group">
                <label for="invoice_date" class="form-label">Date de la facture :</label>
                <input type="date" class="form-input" id="invoice_date" name="invoice_date" value="<?= htmlspecialchars($date) ?>" required>
            </div>

            <div class="form-group">
                <label for="invoice_amount" class="form-label">Montant de la facture :</label>
                <input type="number" class="form-input" min="0" step="0.01" id="invoice_amount" name="invoice_amount" value="<?= htmlspecialchars($amount) ?>" required>
            </div>

            <div class="form-group">
                <label for="invoice_account_number" class="form-label">Numéro de compte :</label>
                <input type="text" class="form-input" id="invoice_account_number" name="invoice_account_number" value="<?= htmlspecialchars($account_number) ?>" required>
            </div>

            <div class="form-group">
                <label for="invoice_sub_account_number" class="form-label">Numéro de sous-compte :</label>
                <input type="text" class="form-input" id="invoice_sub_account_number" name="invoice_sub_account_number" value="<?= htmlspecialchars($sub_account_number) ?>">
            </div>
            ...
            <div class="form-group">
                ...
                <select name="invoice_provider_id" id="invoice_provider_id" class="form-input" required>
                    <?php foreach ($providers as $provider): ?>
                        <option value="<?= htmlspecialchars($provider['provider_id']) ?>" <?= $provider['provider_id'] == $provider_id ? 'selected' : '' ?>>
