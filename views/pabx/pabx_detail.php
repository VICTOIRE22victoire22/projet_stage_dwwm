    ...
    <section class="section-common">
        <table class="table_detail">
            <thead>
                <tr>
                    <th class="label">Marque du PABX:</th>
                    <th class="label">Modèle du PABX:</th>
                    <th class="label">Numéro de série:</th>
            ...
            <tbody>
                <tr>
                    <td class="value"><?=htmlspecialchars($pabx['pabx_brand'])?></td>
                    <td class="value"><?=htmlspecialchars($pabx['pabx_model'])?></td>
                    <td class="value"><?=htmlspecialchars($pabx['pabx_series_number'])?></td>
                    <td class="value"><?=htmlspecialchars($pabx['building_name'] ?? 'N/A')?></td>
                    <?php if(in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
                        <td class="actions">
                            <span class="action-icon">
                                <a href="/index.php?page=pabx/edit&id=<?=$pabx['pabx_id'] ?>">
    ...
    <p class="return_link">
        <a href="/index.php?page=pabx/index&search=<?= urlencode($_GET['search'] ?? '') ?>&sort=<?= urlencode($_GET['sort'] ?? '') ?>&order=<?= urlencode($_GET['order'] ?? 'asc') ?>&page_number=<?= urlencode($_GET['page_number'] ?? 1) ?>&limit=<?= urlencode($_GET['limit'] ?? 10) ?>" class="return_link-detail">
