    ...
    <h1 class='title_detail'>Détail d'un opérateur</h1>

    <!-- Lien vers le formulaire d’ajout -->
    <?php if(in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
        <a href="/index.php?page=provider/add" class="add-form">
    ...
    <section class="section-common">
        <table class="table_detail">
            <thead>
                <tr>
                    <th class="label">Nom de l'opérateur:</th>
                    <th class="label">Offres associées à l'opérateur :</th>
                    <th class="label">Factures associées à l'opérateur :</th>
                    <?php if (in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
            ...
            <tbody>
                <tr>
                    <td class="value"><?=$provider['provider_name']?></td>
                    <td class="value">
                        <?php if (empty($offers)): ?>
                        ...
                        <?php else: ?>
                            <ul class="offer-list">
                                <?php foreach ($offers as $offer): ?>
                                    <li>
                                        <a href="/index.php?page=offer/detail&id=<?= urlencode($offer['offer_id']) ?>" class="table-link" >
                        ...
                        <?php else: ?>
                            <ul class="invoice-list">
                                <?php foreach ($invoices as $invoice): ?>
                                    <li>
                                        <a href="/index.php?page=invoice/detail&id=<?= urlencode($invoice['invoice_id']) ?>" class="table-link" >
                    ...
                    <?php if (in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
                        <td class="actions">
                            <span class="action-icon">
                                <a href="/index.php?page=provider/edit&id=<?=$provider['provider_id'] ?>">
    ...
    <p class="return_link">
        <a href="/index.php?page=provider/index&search=<?= urlencode($_GET['search'] ?? '') ?>&sort=<?= urlencode($_GET['sort'] ?? '') ?>&order=<?= urlencode($_GET['order'] ?? 'asc') ?>&page_number=<?= urlencode($_GET['page_number'] ?? 1) ?>&limit=<?= urlencode($_GET['limit'] ?? 10) ?>" class="return_link-detail">
