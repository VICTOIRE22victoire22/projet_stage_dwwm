    ...
    <h1 class='title_detail'>DETAIL TELEPHONE FIXES</h1>

    <!-- Lien vers le formulaire d’ajout -->
    <?php if(in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
        <a href="/index.php?page=phone/add" class="add-form">
    ...
    <section class="section-common">
        <table class="table_detail">
            <thead>
                <tr>
                    <th class="label">Marque du téléphone fixe:</th>
                    <th class="label">Modèle du téléphone fixe:</th>
                    <th class="label">Statut du téléphone fixe:</th>
                    <th class="label">Ligne téléphonique:</th>
                    <th class="label">Agent associé au téléphone fixe :</th>
                    <th class="label">Bâtiment associé au téléphone fixe :</th>
                    <?php if (in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
            ...
            <tbody>
                <tr>
                    <td class="value"><?=$phone['phone_brand']?></td>
                    <td class="value"><?=$phone['phone_model']?></td>
                    <td class="value"><?=$phone['phone_status']?></td>
                    <td class="value">
                        <?=empty($phone['phone_line_number']) ? 'N/A' : $phone['phone_line_number']?>
                    </td>
                    <td class="value">
                        <?=empty($phone['agent_fullname']) ? 'N/A' : $phone['agent_fullname']?>
                    </td>
                    <td class="value">
                        <?=empty($phone['building_name']) ? 'N/A' : $phone['building_name']?>
                    </td>
                    <?php if (in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
                        <td class="actions">
                            <span class="action-icon">
                                <a href="/index.php?page=phone/edit&id=<?=$phone['phone_id'] ?>">
    ...
    <p class="return_link">
        <a href="/index.php?page=phone/index&search=<?= urlencode($_GET['search'] ?? '') ?>&sort=<?= urlencode($_GET['sort'] ?? '') ?>&order=<?= urlencode($_GET['order'] ?? 'asc') ?>&page_number=<?= urlencode($_GET['page_number'] ?? 1) ?>&limit=<?= urlencode($_GET['limit'] ?? 10) ?>" class="return_link-detail">
