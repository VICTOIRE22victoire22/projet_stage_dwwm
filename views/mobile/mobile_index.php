<!--
    Vue partielle affichant la liste complète des mobiles depuis la base de données.

    Intégrée dynamiquement dans `index.php` (pas d’en-tête HTML ici).
    Les données PHP ($mobiles, $colonnes, $search, $sort, $order...) sont injectées par le contrôleur.

    Fonctionnalités :
    - Affichage, recherche, tri et pagination des mobiles.
...
<h2 class="title">
    <svg class="icon" width="32" height="32" viewBox="0 0 24 24">
        <path fill="#030303" d="M10.5 20h3q.2 0 .35-.15t.15-.35t-.15-.35t-.35-.15h-3q-.2 0-.35.15t-.15.35t.15.35t.35.15M7 23q-.825 0-1.412-.587T5 21V3q0-.825.588-1.412T7 1h10q.825 0 1.413.588T19 3v18q0 .825-.587 1.413T17 23zm0-7h10V6H7z"/>
    </svg>
    TELEPHONES MOBILES
</h2>
...
<?php if(in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
    <a href="/index.php?page=mobile/add&search=<?= urlencode($_GET['search'] ?? '') ?>&sort=<?= urlencode($_GET['sort'] ?? '') ?>&order=<?= urlencode($_GET['order'] ?? 'asc') ?>&page_number=<?= urlencode($_GET['page_number'] ?? 1) ?>&limit=<?= urlencode($_GET['limit'] ?? 10) ?>" class="add-form">
        ...
        &nbsp Ajouter un téléphone mobile
    </a>
<?php endif; ?>
...
<?php if (!empty($mobiles)): ?>
    ...
    <form method="GET" action="/index.php" class="search-form">
        <input type="hidden" name="page" value="mobile/index">
        <input type="text" name="search" placeholder="Rechercher un mobile..." value="<?= htmlspecialchars($search ?? '') ?>" />
        <input type="hidden" name="sort" value="<?= htmlspecialchars($sort ?? 'mobile_brand') ?>">
        <input type="hidden" name="order" value="<?= htmlspecialchars($order ?? 'asc') ?>">
        <button type="submit">Rechercher</button>
        <a href="/index.php?page=mobile/index" class="btn-reset">Réinitialiser</a>
    </form>
    ...
    <div class="items">
        <?php if(!empty($indexSort->search)): ?>
            <p class ="items-number">
                Nombre total de mobiles(s) pour votre recherche <?= htmlspecialchars($indexSort->search) ?> : <?= htmlspecialchars($indexSort->totalResults) ?>
            </p>
        <?php else: ?>
            <p class= "items-number">
                Nombre total de mobile(s) enregistrés : <?= htmlspecialchars($indexSort->totalResults) ?>
            </p>
        <?php endif; ?>
    </div>

    <!-- Tableau des mobiles avec tri et actions -->
    <table>
        <thead>
            <tr>
            ...
            <tr>
                <?php foreach (array_keys($colonnes) as $colonne): ?>
                    <td data-label="<?= htmlspecialchars($colonnes[$colonne] ?? '') ?>">
                        <?php
                            // Vérifie si c'est la colonne mobile_reconditioned
                            if ($colonne === 'mobile_reconditioned') {
                                echo isset($mobile[$colonne]) && $mobile[$colonne] ? 'Reconditionné' : 'Neuf';
                            } else {
                                echo htmlspecialchars($mobile[$colonne] ?? '');
                            }
                        ?>
                    </td>
                <?php endforeach; ?>
                ...
                <td class="actions">
                    <span class="action-icon">
                        <a href="/index.php?page=mobile/detail&id=<?=$mobile['mobile_id'] ?>&search=<?= urlencode($_GET['search'] ?? '') ?>&sort=<?= urlencode($_GET['sort'] ?? '') ?>&order=<?= urlencode($_GET['order'] ?? 'asc') ?>&page_number=<?= urlencode($_GET['page_number'] ?? 1) ?>&limit=<?= urlencode($_GET['limit'] ?? 10) ?>">
                    ...
                    <?php if(in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
                        <span class="action-icon">
                            <a href="/index.php?page=mobile/edit&id=<?=$mobile['mobile_id'] ?>&search=<?= urlencode($_GET['search'] ?? '') ?>&sort=<?= urlencode($_GET['sort'] ?? '') ?>&order=<?= urlencode($_GET['order'] ?? 'asc') ?>&page_number=<?= urlencode($_GET['page_number'] ?? 1) ?>&limit=<?= urlencode($_GET['limit'] ?? 10) ?>">
                                <svg  width="24" height="24" viewBox="0 0 24 24">
                    ...
                    <?php if(in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
                        <span class="action-icon">
                            <form method="POST" action="/index.php?page=mobile/index&<?= http_build_query($_GET) ?>">
                                <input type="hidden" name="mobile_id" value="<?= htmlspecialchars($mobile['mobile_id']) ?>">
                                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrf_token) ?>">
                                <input type="hidden" name="returnLink" value="<?= htmlspecialchars($_SERVER['REQUEST_URI']) ?>">
                                <button type="submit" name="delete_mobile" class="btn-delete"
                                    onclick="return confirm('Voulez-vous vraiment supprimer ce téléphone mobile ?');">
                                    <!-- icône corbeille -->
                                    <svg width="24" height="24" viewBox="0 0 24 24">
...
<?php else: ?>
    <p>Aucun mobile trouvé en base de données.</p>
    <div>
        <a href="/index.php?page=mobile/index" class="return_link-index">
