...
-->
        ...
        <table class="table_detail">
            ...
            <tbody>
                <tr>
                    ...
                    <td class="value table_detail-info"><?=htmlspecialchars($agent['agent_firstname'])?></td>
                    <td class="value table_detail-info"><?=htmlspecialchars($agent['agent_lastname'])?></td>
                    <td class="value table_detail-info"><?=htmlspecialchars($agent['agent_service'])?></td>
                    <?php if (in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
                        <td class="actions">
                            <span class="action-icon">
                                <a href="/index.php?page=agent/edit&id=<?=$agent['agent_id'] ?>">
    ...
    <p class="return_link">
        <a href="/index.php?page=agent/index&search=<?= urlencode($_GET['search'] ?? '') ?>&sort=<?= urlencode($_GET['sort'] ?? '') ?>&order=<?= urlencode($_GET['order'] ?? 'asc') ?>&page_number=<?= urlencode($_GET['page_number'] ?? 1) ?>&limit=<?= urlencode($_GET['limit'] ?? 10) ?>" class="return_link-detail">
