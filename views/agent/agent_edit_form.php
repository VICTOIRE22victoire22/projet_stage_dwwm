<!-- 
    Vue affichant le formulaire d’édition d’un agent.

    Les champs du formulaire sont préremplis grâce aux variables PHP
    contenant les données existantes issues de la base.
-->
    ...
    <section class="form-container">
        <!-- Formulaire de mise à jour des données d’un agent -->
        <form method="POST" action="">
            <div class="form-group">
                <label for="agent_firstname" class="form-label">Prénom :</label>
                <input type="text" class="form-input" id="agent_firstname" name="agent_firstname" 
                       value="<?= htmlspecialchars($firstname) ?>" required>
            </div>

            <div class="form-group">
                <label for="agent_lastname" class="form-label">Nom :</label>
                <input type="text" class="form-input" id="agent_lastname" name="agent_lastname" 
                    value="<?= htmlspecialchars($lastname) ?>" required>
            </div>

            <div class="form-group">
                <label for="agent_service" class="form-label">Service :</label>
                <input type="text" class="form-input" id="agent_service" name="agent_service" 
                    value="<?= htmlspecialchars($service) ?>" required>
            </div>
            ...
            <div class="form-actions">
                <input type="submit" class="form-submit-btn" value="Enregistrer les modifications">
            </div>
        </form>
        <!-- Lien de retour vers la liste des agents -->
        <p class="return_link">
            <a href="/index.php?page=agent/index&search=<?= urlencode($_GET['search'] ?? '') ?>&sort=<?= urlencode($_GET['sort'] ?? '') ?>&order=<?= urlencode($_GET['order'] ?? 'asc') ?>&page_number=<?= urlencode($_GET['page_number'] ?? 1) ?>&limit=<?= urlencode($_GET['limit'] ?? 10) ?>" class="return_link-form" class='return_link-form'>
