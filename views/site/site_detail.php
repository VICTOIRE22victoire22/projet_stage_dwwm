    ...
    <h1 class='title_detail'>Détail du site</h1>

    <!-- Lien vers le formulaire d’ajout -->
    <?php if(in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
        <a href="/index.php?page=site/add" class="add-form">
    ...
    <section class="section-common">
        <table class="table_detail">
            ...
            <tbody>
                <tr>
                    <td class="value"><?=$site['site_name']?></td>
                    <td class="value">
                        ...
                        <?php else: ?>
                            <ul class='building-list'>
                                <?php foreach ($buildings as $building): ?>
                                    <li>
                                        <a href="/index.php?page=building/detail&id=<?= urlencode($building['building_id']) ?>" class="table-link">
                    ...
                    <?php if (in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
                        <td class="actions">
                            <span class="action-icon">
                                <a href="/index.php?page=site/edit&id=<?=$site['site_id'] ?>">
    ...
    <p class="return_link">
        <a href="/index.php?page=site/index&search=<?= urlencode($_GET['search'] ?? '') ?>&sort=<?= urlencode($_GET['sort'] ?? '') ?>&order=<?= urlencode($_GET['order'] ?? 'asc') ?>&page_number=<?= urlencode($_GET['page_number'] ?? 1) ?>&limit=<?= urlencode($_GET['limit'] ?? 10) ?>" class="return_link-detail">
