    ...
    <h1 class='title_detail'>Détail d'un utilisateur</h1>

    <?php if(in_array($_SESSION['user_role'], ['super-admin'])): ?>
        <a href="/index.php?page=users/add" class="add-form">
        ...
        <table class="table_detail">
            <thead>
                <tr>
                    ...
                    <th class="label">Adresse mail:</th>
                    <th class="label">Identifiant:</th>
                    <th class="label">Rôle de l'utilisateur:</th>
            ...
            <tbody>
                <tr>
                    <td class="value"><?=htmlspecialchars($user['user_firstname'])?></td>
                    <td class="value"><?=htmlspecialchars($user['user_lastname'])?></td>
                    <td class="value"><?=htmlspecialchars($user['user_email'])?></td>
                    <td class="value"><?=htmlspecialchars($user['user_login'])?></td>
                    <td class="value"><?=htmlspecialchars($user['user_role'])?></td>
                    <?php if (in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
                        <td class="actions">
                            <span class="action-icon">
                                <a href="/index.php?page=users/edit&id=<?=$user['user_id'] ?>">
   ...
   <!-- permet le retour à la liste -->
    <p class="return_link">
        <a href="/index.php?page=users/index&search=<?= urlencode($_GET['search'] ?? '') ?>&sort=<?= urlencode($_GET['sort'] ?? '') ?>&order=<?= urlencode($_GET['order'] ?? 'asc') ?>&page_number=<?= urlencode($_GET['page_number'] ?? 1) ?>&limit=<?= urlencode($_GET['limit'] ?? 10) ?>" class="return_link-detail">
