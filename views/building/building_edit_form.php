<!-- 
    Vue affichant le formulaire d'édition d'un bâtiment
...
-->
  ...
  <h1 class="form-title">
    ...
    <section class="form-container">
        <!-- Formulaire de mise à jour des données d’un bâtiment -->
        <form method="POST" action="">

            <div class="form-group">
                <label for="building_name" class="form-label">Nom du bâtiment :</label>
                <input type="text" class="form-input" id="building_name" name="building_name" value="<?= htmlspecialchars($name) ?>" required>
            </div>

            <div class="form-group">
                <label for="building_address" class="form-label">Adresse :</label>
                <input type="text" class="form-input" id="building_address" name="building_address" value="<?= htmlspecialchars($address) ?>" required>
            </div>

            <div class="form-group">
                <label for="building_erp_category" class="form-label">Catégorie ERP :</label>
                <input type="number" class="form-input" id="building_erp_category" name="building_erp_category" value="<?= htmlspecialchars($erp_category) ?>" required>
            </div>
            ...
            <div class="form-group">
                ...
                <select name="building_site_id" id="building_site_id" class="form-input" required>
                    <?php foreach ($sites as $site): ?>
                        <option value="<?= htmlspecialchars($site['site_id']) ?>" <?= $site['site_id'] === $site_id ? 'selected' : '' ?>>
        ...
        <!-- Lien de retour vers la liste des bâtiments -->
        <p class="return_link">
