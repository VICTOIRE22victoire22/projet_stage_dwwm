<!--
    Vue partielle affichant la liste complète des bâtiments depuis la base de données.

    Intégrée dynamiquement dans 'index.php' (pas d'en-tête HTML ici).
    Les données PHP ($buildings, $colonnes, $search, $sort, $order...) sont injectées par le contrôleur.

    Fonctionnalités :
    - Affichage, recherche, tri et pagination des bâtiments.
...
<h2 class="title">
    ...
    <svg class="icon" width="32" height="32" viewBox="0 0 24 24">
        <path fill="#030303" d="M1 11v10h5v-6h4v6h5V11L8 6z"/>
        <path fill="#030303" d="M10 3v1.97l7 5V11h2v2h-2v2h2v2h-2v4h6V3zm9 6h-2V7h2z"/>
    </svg>
    BATIMENTS
</h2>
...
<?php if(in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
    <a href="/index.php?page=building/add&search=<?= urlencode($_GET['search'] ?? '') ?>&sort=<?= urlencode($_GET['sort'] ?? '') ?>&order=<?= urlencode($_GET['order'] ?? 'asc') ?>&page_number=<?= urlencode($_GET['page_number'] ?? 1) ?>&limit=<?= urlencode($_GET['limit'] ?? 10) ?>" class="add-form">
...
<?php if (!empty($buildings)): ?>
    ...
    <form method="GET" action="/index.php" class="search-form">
        <input type="hidden" name="page" value="building/index">
        <input type="text" name="search" placeholder="Rechercher un bâtiment..." value="<?= htmlspecialchars($search ?? '') ?>" />
        <input type="hidden" name="sort" value="<?= htmlspecialchars($sort ?? 'building_name') ?>">
        <input type="hidden" name="order" value="<?= htmlspecialchars($order ?? 'asc') ?>">
        <button type="submit">Rechercher</button>
        <a href="/index.php?page=building/index" class="btn-reset">Réinitialiser</a>
    </form>
    ...
    <div class="items">
        <?php if(!empty($indexSort->search)): ?>
            <p class ="items-number">
                Nombre total de bâtiment(s) pour votre recherche <?= htmlspecialchars($indexSort->search) ?> : <?= htmlspecialchars($indexSort->totalResults) ?>
            </p>
        <?php else: ?>
            <p class= "items-number">
                Nombre total de bâtiment(s) enregistrés : <?= htmlspecialchars($indexSort->totalResults) ?>
            </p>
        <?php endif; ?>
    </div>

    <!-- Tableau des bâtiments avec tri et actions -->
    <table>
        <thead>
            <tr>
            ...
            <tr>
                <?php foreach (array_keys($colonnes) as $colonne): ?>
                    <td data-label="<?= htmlspecialchars($colonnes[$colonne]) ?>">
                        <!-- <?= htmlspecialchars($building[$colonne] ?? '') ?> --> <!--ce code a été rajouté pour afficher la photo du bâtiment-->
                        <?= $building[$colonne] ?>    <!-- enlever ce code -->
                    </td>
                <?php endforeach; ?>
                <td class="actions">
                    <span class="action-icon">
                        <a href="/index.php?page=building/detail&id=<?=$building['building_id'] ?>&search=<?= urlencode($_GET['search'] ?? '') ?>&sort=<?= urlencode($_GET['sort'] ?? '') ?>&order=<?= urlencode($_GET['order'] ?? 'asc') ?>&page_number=<?= urlencode($_GET['page_number'] ?? 1) ?>&limit=<?= urlencode($_GET['limit'] ?? 10) ?>">
                    ...
                    <?php if(in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
                        <span class="action-icon">
                            <a href="/index.php?page=building/edit&id=<?=$building['building_id'] ?>&search=<?= urlencode($_GET['search'] ?? '') ?>&sort=<?= urlencode($_GET['sort'] ?? '') ?>&order=<?= urlencode($_GET['order'] ?? 'asc') ?>&page_number=<?= urlencode($_GET['page_number'] ?? 1) ?>&limit=<?= urlencode($_GET['limit'] ?? 10) ?>">
                                <svg width="24" height="24" viewBox="0 0 24 24">
                    ...
                    <?php if(in_array($_SESSION['user_role'], ['admin', 'super-admin'])): ?>
                        <span class="action-icon">
                            <form method="POST" action="/index.php?page=building/index&<?= http_build_query($_GET) ?>">
...
<?php else: ?>
    <p>Aucun bâtiment trouvé en base de données.</p>
    <div>
        <a href="/index.php?page=building/index" class="return_link-index">
