    ...
    <section class="form-container">
        <form method="POST" action="">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-input" id="offer_name" name="offer_name" value="<?= htmlspecialchars($offer_name) ?>" required>
            </div>

            <div class="form-group">
                <label for="offer_price" class="form-label">Montant de l'offre :</label>
                <input type="number" class="form-input" min="0" step="0.01" id="offer_price" name="offer_price" value="<?= htmlspecialchars($offer_price) ?>" required>
            </div>
            ...
            <div class="form-group">
                ...
                <select name="offer_context" id="offer_context" class="form-input" required>
                    ...
                    <?php foreach ($offer_contexts as $context): ?>
                        <option value="<?= htmlspecialchars($context) ?>" <?= $offer_context == $context ? 'selected' : '' ?>>
                            <?= ucfirst($context) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="form-group">
                ...
                <select name="offer_type" id="offer_type" class="form-input" required>
                    ...
                    <?php foreach ($offer_types as $type): ?>
                        <option value="<?= htmlspecialchars($type) ?>" <?= $offer_type == $type ? 'selected' : '' ?>>
                            <?= ucfirst($type) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="form-group">
                ...
                <select name="offer_provider_id" id="offer_provider_id" class="form-input" required>
                    <option value="" disabled hidden>Sélectionner un opérateur</option>
                    <?php foreach ($providers as $provider): ?>
                        <option value="<?= htmlspecialchars($provider['provider_id']) ?>" <?= $provider['provider_id'] == $offer_provider_id ? 'selected' : '' ?>>
