<?php
    ...
    // Controller pour la table Provider. Sert d'intermédiaire entre la couche vue (html) et le couche "modèle" (ProviderRepository)
    // Contient la logique métier et contrôle le flux des données.

    require_once __DIR__ . '/../model/provider.php';        // Import du fichier gérant les requêtes SQL.
    ...
    {
        private ProviderRepository $providerRepo;       // Déclaration d'une propriété privée contenant l'instanciation du repository (accès aux données).
        
        // Constructeur permettant la connexion à la BDD.
        public function __construct(PDO $pdo) 
        {

            // Instanciaton du repository avec la connexion PDO passée en argument.
            $this->providerRepo = new ProviderRepository($pdo);
        }

        // -------------- FONCTION AFFICHANT LA LISTE DE TOUS LES OPERATEURS (page provider_index.php). --------------
        public function index() 
        {
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_provider'], $_POST['provider_id'], $_POST['csrf_token'])) {

                // Seuls admin et super-admin peuvent supprimer un opérateur
                ...
                $this->providerRepo->deleteProvider((int)$_POST['provider_id']);
            ...
            // On récupère le nombre total d'opérateurs correspondant à la recherche actuelle.
            // Cela sert notamment pour la pagination afin de savoir combien de pages sont nécessaires.
            $indexSort->totalResults = $this->providerRepo->countAll($indexSort->search);

            // Récupération des opérateurs à afficher avec tri et pagination
            ...
            // clé = nom de la colonne en BDD, valeur = label affiché dans l'entête.
             $colonnes = [
                'provider_name' => "Nom de l'opérateur"
            ];
            ...
            require __DIR__ . '/../views/provider/provider_index.php';
        }

        // -------------- FONCTION AFFICHANT LE DETAIL D'UN OPERATEUR (page provider_detail.php). --------------
        public function detail(int $id) 
        {
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_provider'], $_POST['provider_id'], $_POST['csrf_token'])) {
                ...
                // Redirection vers la page index pour éviter le rechargement du formulaire.
                header("Location: /index.php?page=provider/index");
                exit;       // Stop l'exécution du script.
            }

            // Récupération des informations de l'opérateur depuis la BDD.
            $provider = $this->providerRepo->getById($id);

            // Si aucun opérateur n'est trouvé, on renvoie une erreur 404.
            if (!$provider) {
                http_response_code(404);
                echo "<p>Provider introuvable.</p>";
                exit;
            }

            // Récupération des offres et factures liées à l'opérateur
            $offers = $this->providerRepo->getAllOffers($id);
            $invoices = $this->providerRepo->getAllInvoices($id);

            $csrf_token = CsrfToken::generateToken();

            // Chargement de la vue correspondante (affichage de la page HTML)
            require __DIR__ . '/../views/provider/provider_detail.php';
        }

        // -------------- FONCTION PERMETTANT L'AJOUT D'UN OPERATEUR (page provider_form.php). --------------
        public function add() 
        {
            // Seuls admin et super-admin peuvent ajouter un opérateur
            authorize(['admin', 'super-admin']);
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                $data = [
                    'provider_name' => valide_donnees($_POST['provider_name'])
                ];

                // Si les données sont valides selon la fonction validateProvider()
                if($this->validateProvider($data)) {
                    // Ajout de l'opérateur en base de données.
                    $this->providerRepo->addProvider($data);
                    ...
                    // Redirection vers la liste des opérateurs après ajout
                    header("Location: /index.php?" . http_build_query($params));
                    exit;
            ...
            // Chargement de la vue correspondante (affichage de la page HTML)
            require __DIR__ . '/../views/provider/provider_form.php';
        }

        // -------------- FONCTION PERMETTANT LA MODFICATION D'UN OPERATEUR (page provider_edit_form.php). --------------
        public function edit(int $id) 
        {

            // Seuls admin et super-admin peuvent modifier un opérateur
            authorize(['admin', 'super-admin']);

            // Récupère l'opérateur correspondant à l'ID fourni.
            $provider = $this->providerRepo->getById($id);

            // Si aucun opérateur n'est trouvé, on arrête le script
            if (!$provider) {
                http_response_code(404);
                echo "<p>Provider introuvable.</p>";
                exit;
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                if($this->validateProvider($data)) {
                    ...
                    // Mise à jour de l'opérateur en base de données selon l'ID avec les nouvelles valeurs
                    $this->providerRepo->updateProvider($id, $data);
            ...
            // Chargement de la vue correspondante (affichage de la page HTML)
            require __DIR__ . '/../views/provider/provider_edit_form.php';
        }

        // -------------- FONCTION PRIVEE DE VALIDATION DES DONNEES --------------
        private function validateProvider(array $data): bool 
        {

            return !empty($data['provider_name'])                                        // Le nom ne doit pas être vite.
                   && preg_match("/^[A-Za-zÀ-ÿ '-]+$/", $data['provider_name']);       // Le champs nom ne doit contenir uniquement que des lettres, espaces, apostrophes ou tirets
        }
    }
